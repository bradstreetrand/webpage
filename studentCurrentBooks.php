    ...
    $sql = "SELECT student, first_name, last_name FROM student WHERE username = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                $sql1 = "SELECT book.title, book.author, book.thumbnail, book.description, reading_log.start, book.book FROM student INNER JOIN reading_log ON student.student = reading_log.student INNER JOIN book ON reading_log.book = book.book WHERE student.student = ? AND reading_log.finish IS NULL";
